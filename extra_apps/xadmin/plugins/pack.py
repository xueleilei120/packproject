...
class PackPlugin(BaseAdminPlugin):

    show_pack_btn_fields = []
    show_pack_btn = True

    def result_item(self, item, obj, field_name, row):
        if (self.show_pack_btn or (field_name in self.show_pack_btn_fields)):
            rel_obj = None
            if hasattr(item.field, 'rel') and isinstance(item.field.rel, models.ManyToOneRel):
                rel_obj = getattr(obj, field_name)
            elif field_name in self.show_pack_btn_fields:
                rel_obj = obj
                ...
                try:
                    ...
                    if item_res_uri:
                        if has_change_perm:
                            ...
                            edit_url = ''
                        item.btns.append('<a data-res-uri="%s" data-edit-uri="%s" class="btn btn-submit btn-primary details-handler" rel="tooltip" title="%s">打包</a>'
    ...
    #     if self.show_pack_btn or self.show_pack_btn_fields:
    #         media = media + self.vendor('xadmin.plugin.pack.js', 'xadmin.form.css')
    #     return media

site.register_plugin(PackPlugin, ListAdminView)
